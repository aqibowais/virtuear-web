      ...
      const onTouchMove = (e: TouchEvent) => {
        ...
        try {
          ...
          if (prevAngle !== null) {
            ...
            rec.rotationY += deltaAngle
            const rotQuat = ecs.math.quat.yRadians(rec.rotationY)
            world.setQuaternion(rec.eid, rotQuat.x, rotQuat.y, rotQuat.z, rotQuat.w)
          }

          if (prevDist !== null && prevDist > 0) {
            const scaleFactor = dist / prevDist
            rec.scale = Math.max(MIN_SCALE, Math.min(MAX_SCALE, rec.scale * scaleFactor))
            world.setScale(rec.eid, rec.scale, rec.scale, rec.scale)
          }
