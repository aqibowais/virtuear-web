...
const createVirtualEntryPlugin = require('./entry-plugin.cjs')
const createDev8Plugin = require('./dev8-plugin.cjs')

const rootPath = process.cwd()
const distPath = path.join(rootPath, 'ar-dist')
const srcPath = path.join(rootPath, 'src', 'ar-experience')
...
const makeTsLoader = () => ({
  ...
  options: {
    configFile: path.join(rootPath, 'tsconfig.ar.json'),
  },
})
...
const makeAssetLoader = () => ({
  ...
  loader: path.join(__dirname, 'asset-loader.cjs'),
})
